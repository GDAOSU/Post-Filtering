...
void VLSiftExtractor::Run(cv::Mat & image, std::vector<cv::KeyPoint>& keypoints, cv::Mat & descriptors)
{
	initialize(image.data, image.cols, image.rows);
	run_sift();

	keypoints.resize(numpt_);
	descriptors = cv::Mat(numpt_, numdims_, CV_32FC1, cv::Scalar(0));
	for (size_t i = 0; i < numpt_; i++)
	{
		keypoints[i].pt = cv::Point2f(keypoints_[i][0], keypoints_[i][1]);
		keypoints[i].response = keypoints_[i][2];
		keypoints[i].angle = keypoints_[i][3];
		float* ptr = descriptors.ptr<float>(i);
		for (size_t j = 0; j < numdims_; j++)
		{
			*ptr++ = descriptor_[i][j];
